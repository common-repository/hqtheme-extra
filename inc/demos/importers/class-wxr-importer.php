...
if (!class_exists('WXR_Importer') && class_exists('WP_Importer')) :
    ...
    class WXR_Importer extends WP_Importer {
        ...
        protected function prefill_existing_posts() {
            global $wpdb;
            $posts = $wpdb->get_results("SELECT ID, guid, post_type, post_name FROM {$wpdb->posts}");

            foreach ($posts as $item) {
                if ('nav_menu_item' === $item->post_type) {
                    $this->exists['menu_item'][$item->post_name] = $item->ID;
                } else {
        ...
        protected function post_exists($data) {
            if ('nav_menu_item' === $data['post_type']) {
                if (isset($this->exists['menu_item'][$data['post_name']])) {
                    $exists = true;
                } else {
                    $exists = false;
                    $this->exists['menu_item'][$data['post_name']] = 1;
                }
            } else {
                ...
                if ($this->options['prefill_existing_posts'] && isset($this->exists['post'][$exists_key])) {
                    return true;
                }
